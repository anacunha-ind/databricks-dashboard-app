...
BRANCH_ENDPOINT_RESOURCE="projects/${LAKEBASE_PROJECT}/branches/${BRANCH_NAME}/endpoints/primary"
...
env:
  ...
  - name: LAKEBASE_ENDPOINT
    value: ${BRANCH_ENDPOINT_RESOURCE}
  - name: LAKEBASE_DATABASE
    value: databricks_postgres
APPYAML
echo "   ✓ app.yaml written"

# ── 5. Deploy bundle (creates app resource + app SP) ──────────────────────────
...
# ── 6. Grant Lakebase access to app SP ───────────────────────────────────────
...
if [[ -n "${APP_SP}" ]]; then
  ...
  databricks postgres create-role \
    ...
    && echo "   ✓ Lakebase role created for app SP" \
    || echo "   ⚠ Role creation failed — app may fail to connect to Lakebase"
else
  echo "   ⚠ Could not determine app SP — Lakebase role must be created manually"
fi
